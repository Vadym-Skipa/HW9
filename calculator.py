...
class Calculator:
    """
    Калькулятор працює наступним чином:
        На вхід приймаються 0, 2 або 3 елементи (елемент - операнд(число) або опереатор(математична дія),
            елементи розділяються пробілами).
        Якщо подаються  3 елементи: AA B CC - AA і CC операнди, B - оператор;
                        2 елементи: B CC - тесаме , що й при трьох елементах тільки замість AA буде значення збережене в
                            пам'яті (результат останньої виконаної операції, на початку 0);
                        0 елементів: по факту Enter - скидує значення збережене в пам'яті до 0;
    Список доступних операторів та їх дії:
        AA + CC - повертає суму AA і CC
        AA - CC - повертає різницю між AA і CC
        AA * CC - повертає добуток AA і CC
        AA / CC - повертає частку від ділення AA на CC
        AA ^ CC - повертає значення AA в степені CC
        AA r CC - повертає корінь CC-го степеня з AA
        AA % CC - повертає значення, що рівне CC відсотків від AA
    """
